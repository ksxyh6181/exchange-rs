...
use salvo::prelude::*;
use service::risk_service::RiskService;
use serde::{Deserialize, Serialize};

const SERVICE_NAME: &str = "risk_service";
const SERVICE_REGISTER_NAME: &str = "risk-service";
...
struct RiskServiceConfig {
    ...
    // 可以添加风控服务特有的配置
    pub risk_check_interval_seconds: Option<u64>,
...
async fn main() {
    ...
    let config = {
        ...
        match toml::from_str::<RiskServiceConfig>(&config_content) {
            Ok(config) => {
    ...
    api::risk_handler::init_service(risk_service).await;
    let router = api::risk_handler::routes();
